        ...
        <form onSubmit={submitSearch} className="lg:col-span-2 relative">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setSearch(v)
              // Xoá hết chữ trong ô tìm → bỏ luôn keyword khỏi URL ngay, để khi
              // lọc tiếp theo tác giả không bị dính keyword cũ (AND ra 0 kết quả).
              if (v === '' && keyword) patchParams({ keyword: null, page: null })
            }}
            placeholder="Tìm theo tên truyện…"
            className="w-full pl-9 pr-9 py-2 text-sm bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500/30 text-slate-800 dark:text-slate-100 placeholder-slate-400"
          />
          {search && (
            <button
              type="button"
              ...
              onClick={() => {
                setSearch('')
                if (keyword) patchParams({ keyword: null, page: null })
              }}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-lg leading-none"
            >
              ×
            </button>
          )}
